...
- (void)viewDidLoad {
    [super viewDidLoad];
    TBPerformanceLog(@"Touchbox: Songs viewDidLoad");
    ...
    if (![[TBLibraryManager sharedManager] songsLoaded]) {
        ...
        TBPerformanceLog(@"Touchbox: songs count=%lu table rows=%lu (already loaded)",
...
- (void)songsReady:(NSNotification *)notification {
    self.items = [[TBLibraryManager sharedManager] songs];
    TBPerformanceLog(@"Touchbox: songs count=%lu table rows=%lu (notification)",
          (unsigned long)[self.items count], (unsigned long)[self.items count]);
    self.tableView.backgroundView = nil;
    [self.tableView reloadData];
}
