...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)path { static NSString *identifier = @"RecentKind"; UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier]; if (!cell) cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier] autorelease]; cell.backgroundColor = [TBTheme backgroundColor]; cell.textLabel.font = [TBTheme primaryFont]; cell.textLabel.textColor = [TBTheme primaryTextColor]; cell.textLabel.text = path.row == 0 ? @"Recently Played" : (path.row == 1 ? @"Recently Added" : (path.row == 2 ? @"Theme" : @"Classic Mode")); cell.accessoryType = UITableViewCellAccessoryNone; UIImageView *disclosure = (UIImageView *)cell.accessoryView; if (![disclosure isKindOfClass:[UIImageView class]]) { disclosure = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 20, [TBTheme compactRowHeight])] autorelease]; disclosure.contentMode = UIViewContentModeCenter; cell.accessoryView = disclosure; } disclosure.image = [TBIconFactory iconNamed:@"disclosure" active:NO]; return cell; }
